...
function createArray(...elements) {
  if (elements.length <= 0) {
    console.log('Помилка: немає вхідних елементів');
    return [];
  }
...
function copyAndSwapElements(arr, target, start, end) {
  if (start > arr.length || end > arr.length || start + end < 0) {
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr) || start + end < 0 || start > arr.length || end > arr.length) {
    return false;
  }
...
function customAt(arr, index) {
  ...
  const elementAt = arr.at(index);
  if (typeof(elementAt) == 'number') {
...
function customIncludes(arr, element) {
  ...
  const myIncludes = arr.includes(element);
  console.log(arr.filter((x) => {return x == element;}).length);
  return myIncludes;
  // Перевіряємо, чи вхідний параметр є масивом
